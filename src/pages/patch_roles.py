...
with open('Roles.jsx', 'r', encoding='utf-8') as f:
    content = f.read()

# 1. Inject the Users state and functions
states_target = '''  const [availablePermissions, setAvailablePermissions] = useState([]);
...
states_replacement = '''  const [availablePermissions, setAvailablePermissions] = useState([]);
...
# 2. Inject Tabs and conditional rendering
layout_target = '''  return (
...
# 3. Add closing tags for the roles tab
closing_target = '''      {showRoleModal && ('''
...
with open('Roles.jsx', 'w', encoding='utf-8') as f:
    f.write(content)
