...
const initialStudents = [
  { id: 1, name: 'Aarav Singh', enrollNo: 'OP/23/CE/001', batch: 'Batch B1', status: 'Present' },
  { id: 2, name: 'Vikram Patel', enrollNo: 'OP/23/ME/015', batch: 'Batch B1', status: 'Present' },
  { id: 3, name: 'Jayesh Soni', enrollNo: 'OP/23/CE/104', batch: 'Batch B1', status: 'Absent' },
  { id: 4, name: 'Rahul Joshi', enrollNo: 'OP/23/ME/018', batch: 'Batch B1', status: 'Present' },
];

const PracticalAttendance = () => {
  const [filterBatch, setFilterBatch] = useState('Batch B1');
  const [experimentNo, setExperimentNo] = useState('Exp #4 (Logic Gate Verification)');
  ...
  const handleSave = () => {
    toast.success(`Practical attendance saved for ${experimentNo}!`);
  };
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col h-full font-['Inter']">
      ...
      <div className="p-6 border-b border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-[16px] font-bold text-gray-800">Student Practical Attendance</h2>
          <p className="text-[12px] text-gray-500 mt-0.5 font-medium">Record batch attendance logs and link experiments to teacher verifications</p>
        </div>
      ...
      <div className="p-6 border-b border-gray-100 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          >
            <option value="Batch B1">Batch B1 (3rd Sem)</option>
            <option value="Batch B2">Batch B2 (5th Sem)</option>
          </select>
        </div>
        <div>
          <label className="block text-[12px] font-semibold text-gray-600 mb-1">Experiment Name / Title</label>
          <input 
            type="text" 
            value={experimentNo}
        ...
        <table className="w-full text-left border-collapse min-w-[800px]">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              ...
              <th className="py-4 px-6 text-[12px] font-bold text-gray-800">Practical Attendance</th>
            </tr>
          </thead>
          <tbody>
            {students.map(item => (
                    ...
                    {['Present', 'Absent'].map(status => (
                      <button
                        ...
                        className={`px-3 py-1 rounded text-[11px] font-bold border transition-colors ${
                          item.status === status
                            ? status === 'Present' ? 'bg-green-50 text-green-700 border-green-200 shadow-sm' : 'bg-red-50 text-red-700 border-red-200 shadow-sm'
