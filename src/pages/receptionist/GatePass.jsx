          ...
          <tbody>
            {filtered.map(item => (
              <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                <td className="py-4 px-6 text-[13px] font-semibold text-[#0A6C54]">{item.passNo}</td>
                ...
                <td className="py-4 px-6">
                  {item.status === 'Currently Out' && (
                    <button onClick={() => handleConfirmIn(item.id)} className="px-2 py-1 text-[11px] font-bold bg-[#0A6C54] text-white rounded hover:bg-[#085a46]">Confirm Return</button>
                  )}
