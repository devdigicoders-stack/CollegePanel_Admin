...
const initialVisitors = [
  { id: 1, name: 'Suresh Chandra', mobile: '9988776655', idProof: 'Aadhaar Card', meetPerson: 'Principal Office', purpose: 'Fee concession appeal', entry: '15-Feb 09:30 AM', exit: '15-Feb 10:30 AM', status: 'Checked Out' },
  { id: 2, name: 'Aditya Birla Rep', mobile: '9988776656', idProof: 'Employee ID', meetPerson: 'Placement Officer', purpose: 'Campus placement drive discussion', entry: '15-Feb 10:00 AM', exit: '-', status: 'Inside Campus' },
];
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col h-full font-['Inter']">
      ...
      <div className="p-6 border-b border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-[16px] font-bold text-gray-800">Campus Visitor Pass Management</h2>
        ...
        <div className="flex gap-3">
          ...
          <button onClick={() => setShowAddModal(true)} className="bg-primary hover:bg-primary-hover text-white px-5 py-2.5 rounded-lg text-[13px] font-semibold flex items-center gap-2 transition-colors">
            <Plus size={16} /> Print Visitor Pass
          </button>
        </div>
      </div>
          ...
          <tbody>
            {filtered.map(item => (
              <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                ...
                <td className="py-4 px-6 text-[13px] text-gray-600">{item.purpose}</td>
                ...
                <td className="py-4 px-6">
                  {item.status === 'Inside Campus' && (
                    <button onClick={() => handleCheckout(item.id)} className="px-2 py-1 text-[11px] font-bold bg-primary text-white rounded hover:bg-primary-hover">Checkout</button>
                  )}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full overflow-hidden shadow-xl animate-in fade-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
              <h3 className="font-bold text-gray-800 text-[15px]">Register Visitor Entry Pass</h3>
