...
const ReceptionistReports = () => {
  const [reportType, setReportType] = useState('Visitor Report');
  const [dateRange, setDateRange] = useState({ start: '', end: '' });

  const monthlyVisitorsOptions = {
    chart: { type: 'area', height: 260, backgroundColor: 'transparent' },
    title: { text: '' },
    xAxis: { categories: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'] },
    yAxis: { title: { text: '' } },
    series: [
      { name: 'Visitors Checked-In', data: [120, 240, 180, 290, 310, 380], color: '#3B82F6', fillOpacity: 0.1 }
    ],
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col h-full font-['Inter']">
      ...
      <div className="p-6 border-b border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-[16px] font-bold text-gray-800">Receptionist Audits & Reports</h2>
          <p className="text-[12px] text-gray-500 mt-0.5 font-medium font-semibold">Generate campus visitor rosters, callback summaries, and courier checklists</p>
        </div>
      ...
      <div className="p-6 space-y-6 flex-1 overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-green-50/50 border border-green-100 rounded-xl p-5 flex items-center justify-between">
            <div className="space-y-1">
              <span className="text-[12px] text-gray-500 font-medium font-semibold uppercase tracking-wider">Total Visitors Registered</span>
              <h4 className="text-[20px] font-bold text-green-700">1,570 Visitors</h4>
            </div>
          ...
          <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-5 flex items-center justify-between">
            <div className="space-y-1">
              <span className="text-[12px] text-gray-500 font-medium font-semibold uppercase tracking-wider">Appointments Logged</span>
              <h4 className="text-[20px] font-bold text-blue-700">340 Meetings</h4>
            </div>
          ...
          <div className="bg-red-50/50 border border-red-100 rounded-xl p-5 flex items-center justify-between">
            <div className="space-y-1">
              <span className="text-[12px] text-gray-500 font-medium font-semibold uppercase tracking-wider">Gate Pass Checkouts</span>
              <h4 className="text-[20px] font-bold text-red-700">182 Slips</h4>
            </div>
        ...
        <div className="bg-gray-50/40 p-4 border border-gray-100 rounded-xl">
          <h4 className="font-bold text-gray-800 text-[14px] mb-4 font-semibold uppercase tracking-wider">Monthly Visitor Flow Rate</h4>
          <HCReact highcharts={Highcharts} options={monthlyVisitorsOptions} />
        </div>
      </div>
    </div>
  );
