...
const MyAttendance = () => {
  ...
  const fetchLogs = async () => {
    try {
      setLoading(true);
      const res = await axiosInstance.get('/punch/reports/monthly', {
        params: { month, year }
      });
  ...
  const downloadCSV = () => {
    ...
    link.setAttribute("download", `My_Attendance_${month}_${year}.csv`);
  ...
  return (
    <div className="max-w-6xl mx-auto pb-10 font-['Inter']">
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 font-['Outfit']">My Attendance</h1>
          <p className="text-sm text-gray-500 mt-1">View your monthly attendance report.</p>
        </div>
      </div>
