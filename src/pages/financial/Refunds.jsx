...
const refundsData = [
  { id: 1, refundNo: 'REF/2024/001', enrollNo: 'OP/24/CE/010', name: 'Karan Mehta', course: 'Diploma in CE', reason: 'Admission Cancellation', requestDate: '2024-02-20', totalPaid: 18500, deduction: 5000, refundAmount: 13500, payMode: 'Bank Transfer', bankDetails: 'SBI - XXXX1234', status: 'Approved', txnRef: null },
  { id: 2, refundNo: 'REF/2024/002', enrollNo: 'OP/24/IT/015', name: 'Riya Shah', course: 'Diploma in IT', reason: 'Duplicate Payment', requestDate: '2024-02-18', totalPaid: 5000, deduction: 0, refundAmount: 5000, payMode: 'UPI', bankDetails: 'UPI - riya@upi', status: 'Completed', txnRef: 'TXN20240218001' },
  { id: 3, refundNo: 'REF/2024/003', enrollNo: 'OP/24/ME/020', name: 'Dev Patel', course: 'Diploma in ME', reason: 'Hostel Cancellation', requestDate: '2024-02-15', totalPaid: 12000, deduction: 2000, refundAmount: 10000, payMode: 'Cheque', bankDetails: 'HDFC - XXXX5678', status: 'Under Review', txnRef: null },
  { id: 4, refundNo: 'REF/2024/004', enrollNo: 'OP/24/EE/008', name: 'Anita Verma', course: 'Diploma in EE', reason: 'Excess Payment', requestDate: '2024-02-12', totalPaid: 3000, deduction: 0, refundAmount: 3000, payMode: 'UPI', bankDetails: 'UPI - anita@upi', status: 'Processing', txnRef: null },
  { id: 5, refundNo: 'REF/2024/005', enrollNo: 'OP/24/CE/025', name: 'Suresh Kumar', course: 'Diploma in CE', reason: 'Security Deposit', requestDate: '2024-02-10', totalPaid: 5000, deduction: 0, refundAmount: 5000, payMode: 'Bank Transfer', bankDetails: 'PNB - XXXX9012', status: 'Requested', txnRef: null },
  { id: 6, refundNo: 'REF/2024/006', enrollNo: 'OP/24/IT/030', name: 'Meera Joshi', course: 'Diploma in IT', reason: 'Transport Cancellation', requestDate: '2024-02-08', totalPaid: 8000, deduction: 1000, refundAmount: 7000, payMode: 'Bank Transfer', bankDetails: 'BOB - XXXX3456', status: 'Rejected', txnRef: null },
];
...
const Refunds = () => {
  ...
  const filtered = refundsData.filter(r => {
    const matchSearch = r.name.toLowerCase().includes(search.toLowerCase()) || r.refundNo.toLowerCase().includes(search.toLowerCase());
    const matchStatus = filterStatus === 'All' || r.status === filterStatus;
    return matchSearch && matchStatus;
  });

  const totalRefundAmount = filtered.reduce((sum, r) => sum + r.refundAmount, 0);
  const pendingCount = filtered.filter(r => ['Requested', 'Under Review', 'Approved', 'Processing'].includes(r.status)).length;
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col h-full font-['Inter']">
      ...
      <div className="px-6 py-4 border-b border-gray-100 grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Completed', value: refundsData.filter(r => r.status === 'Completed').length, color: 'bg-green-50 text-green-700' },
          { label: 'Total Requests', value: refundsData.length, color: 'bg-blue-50 text-blue-700' },
        ].map(card => (
          ...
          <tbody>
            {filtered.map(r => (
              <tr key={r.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                ...
                <td className="py-3 px-4 text-[13px] text-gray-700">₹{r.totalPaid.toLocaleString()}</td>
                <td className="py-3 px-4 text-[13px] text-red-600">₹{r.deduction.toLocaleString()}</td>
                <td className="py-3 px-4 text-[13px] font-semibold text-green-700">₹{r.refundAmount.toLocaleString()}</td>
          ...
          <div className="bg-white rounded-2xl w-full max-w-xl shadow-xl">
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="number" placeholder="Enter deduction" className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-[13px] focus:outline-none focus:ring-1 focus:ring-[#0A6C54]" />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="text" placeholder="Bank name, account no. or UPI ID" className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-[13px] focus:outline-none focus:ring-1 focus:ring-[#0A6C54]" />
              </div>
            </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="bg-gray-50 rounded-xl p-4 grid grid-cols-3 gap-4">
                <div className="text-center">
                  ...
                  <p className="text-[16px] font-bold text-gray-800">₹{viewRefund.totalPaid.toLocaleString()}</p>
                </div>
                <div className="text-center">
                  <p className="text-[11px] text-gray-500">Deduction</p>
                  <p className="text-[16px] font-bold text-red-600">₹{viewRefund.deduction.toLocaleString()}</p>
                </div>
                <div className="text-center">
                  <p className="text-[11px] text-gray-500">Refund Amount</p>
                  <p className="text-[16px] font-bold text-green-700">₹{viewRefund.refundAmount.toLocaleString()}</p>
                </div>
              </div>
