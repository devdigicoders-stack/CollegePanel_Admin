...
import { Search, Download, Bell, CheckCircle, XCircle } from 'lucide-react';
...
const Reservations = () => {
  ...
  const handleExport = () => {
    ...
    const exportData = filtered.map(r => ({
      'Request ID': r.reserveId,
      'Book Title': r.bookId?.title || 'Unknown',
      'Reserved For': r.memberId ? `${r.memberId.firstName} ${r.memberId.lastName || ''}` : 'Unknown',
      'Request Date': new Date(r.requestDate).toLocaleDateString(),
      'Queue Position': r.queuePosition > 0 ? `#${r.queuePosition}` : '-',
      'Status': r.status
    }));
    ...
    XLSX.writeFile(workbook, 'Library_Reservations.xlsx');
  };
