...
const ScholarshipReports = () => {
  const [reportType, setReportType] = useState('Scholarship-wise Report');
  const [dateRange, setDateRange] = useState({ start: '', end: '' });

  const monthlyFundsOptions = {
    chart: { type: 'area', height: 260, backgroundColor: 'transparent' },
    title: { text: '' },
    xAxis: { categories: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'] },
    yAxis: { title: { text: '' } },
    series: [
      { name: 'Funds Disbursed (₹)', data: [120000, 250000, 480000, 720000, 880000, 1120000], color: '#0A6C54', fillOpacity: 0.1 }
    ],
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col h-full font-['Inter']">
      ...
      <div className="p-6 border-b border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-[16px] font-bold text-gray-800">Scholarship Audits & Reports</h2>
          <p className="text-[12px] text-gray-500 mt-0.5 font-medium font-semibold">Generate scheme-wise statements, total fund disbursement logs, and renewal counts</p>
        </div>
      ...
      <div className="p-6 space-y-6 flex-1 overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-green-50/50 border border-green-100 rounded-xl p-5 flex items-center justify-between">
            <div className="space-y-1">
              <span className="text-[12px] text-gray-500 font-medium font-semibold uppercase tracking-wider">Total Fund Disbursed</span>
              <h4 className="text-[20px] font-bold text-green-700">₹32,00,000</h4>
            </div>
          ...
          <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-5 flex items-center justify-between">
            <div className="space-y-1">
              <span className="text-[12px] text-gray-500 font-medium font-semibold uppercase tracking-wider">Scholarship Sanctioned</span>
              <h4 className="text-[20px] font-bold text-blue-700">₹45,50,000</h4>
            </div>
          ...
          <div className="bg-red-50/50 border border-red-100 rounded-xl p-5 flex items-center justify-between">
            <div className="space-y-1">
              <span className="text-[12px] text-gray-500 font-medium font-semibold uppercase tracking-wider">Rejected Applications</span>
              <h4 className="text-[20px] font-bold text-red-700">58 Cases</h4>
            </div>
        ...
        <div className="bg-gray-50/40 p-4 border border-gray-100 rounded-xl">
          <h4 className="font-bold text-gray-800 text-[14px] mb-4 font-semibold uppercase tracking-wider">Monthly Funds Disbursed Trend</h4>
          <HCReact highcharts={Highcharts} options={monthlyFundsOptions} />
        </div>
      </div>
    </div>
  );
